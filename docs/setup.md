...
| Setting                   | Description                                                                                                                                                                                               | Type                        |
|---------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|-----------------------------|
| `USE_TOKENS`              | If True, can authenticate<br>with tokens in Authorization<br>header. Set this to False and<br>`USE_COOKIES` to True to only<br>allow cookie authentication.                                               | bool                        |
| `USE_COOKIES`             | If True, can authenticate<br>with tokens in HttpOnly<br>headers. Cookies will be<br>checked before Authorization<br>headers if they are<br>enabled.                                                       | bool                        |
| `DEFAULT_LOGIN_METHOD`    | Default login method<br>to use if none is given in<br>Prefer-headers. If not set,<br>cookie-based login will be<br>used if enabled,<br>else token-based.                                                  |                             |
| `ACCESS_TOKEN_KEY`        | Cookie key to use<br>for the access token                                                                                                                                                                 | str                         |
| `REFRESH_TOKEN_KEY`       | Cookie key to use<br>for the refresh token                                                                                                                                                                | str                         |
